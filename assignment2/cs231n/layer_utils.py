...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for i in range(newH):
            for j in range(newW):
                for f in range(F):
                    current_block = x_pad[n, :, i * stride : i * stride + HH, j * stride : j * stride + WW]
                    new_val = np.sum(current_block * w[f])
                    out[n, f, i, j] = new_val
                out[n, :, i, j] += b
    #############################################################################
    #                             END OF YOUR CODE                              #
    ...
    for n in range(N):
        for i in range(H_out):
            for j in range(W_out):
                for f in range(F):
                    current_block = x_pad[n, :, i * stride : i*stride + HH, j * stride: j*stride + WW]
                    dw[f] += current_block * dout[n, f, i, j]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    current_block = x[n, c, i*stride:i*stride+pool_h, j*stride:j*stride+pool_w]
                    curr_max = np.max(current_block)
                    out[n, c, i, j] = curr_max
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    current_block = x[n, c, i*stride:i*stride+pool_h, j*stride:j*stride+pool_w]
                    curr_max = np.max(current_block)
                    x_mask = current_block == curr_max
                    dx[n, c, i*stride:i*stride+pool_h, j*stride:j*stride+pool_w] += dout[n, c, i, j] * x_mask
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    #############################################################################
    N, C, H, W = x.shape
    batch_forw, cache = batchnorm_forward(x.transpose(0,3,2,1).reshape((N*H*W), C), gamma, beta, bn_param)
    out = batch_forw.reshape(N, W, H, C).transpose(0,3,2,1)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
